...
/*
        ...
        /// <returns></returns>
        public string GetRequestToken(string dbid, string desc, decimal amount)
        {
            PlatformApiXmlHttpPostRequest request = CreateAuthenticatedApiRequest("API_GetDBvar", dbid, false);
            request.ReqDoc.AddTextParameter("desc", desc);
            request.ReqDoc.AddTextParameter("amount", amount.ToString());
            XmlDocument respXml = request.ExecuteRequest(WorkNotification);
            string returnValue = String.Empty;

            XmlNode node = respXml.SelectSingleNode("//RequestToken");
            if (node != null)
            {
                returnValue = node.InnerText;
            }
        ...
        /// <param name="amount">Amount.</param>
        /// <returns></returns>
        public string Charge(string dbid, string desc, decimal amount)
        {
            PlatformApiXmlHttpPostRequest request = CreateAuthenticatedApiRequest("API_GetDBvar", dbid, false);
            request.ReqDoc.AddTextParameter("desc", desc);
            request.ReqDoc.AddTextParameter("amount", amount.ToString());
