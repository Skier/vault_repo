...
namespace Weborb.Samples.Email
{
    public class Message
    {
        ...
        public static MessageInfo Convert(IndyMessage indyMsg, int size, int accountId)
        {
            ...
            return new MessageInfo(uid, indyMsg.From.Text, 
                                   indyMsg.Recipients.EMailAddresses, 
                                   indyMsg.CCList.EMailAddresses,
                                   indyMsg.BccList.EMailAddresses,
                                   GetSubject(indyMsg),
                                   inReplyTo, size, 
        ...
        public static IndyMessage Convert(MessageInfo message)
        {
            ...
            indyMessage.From = new EMailAddressItem(message.From);
            indyMessage.Recipients.Add().Text = message.To;
            indyMessage.CCList.Add().Text = message.CC;
            indyMessage.BccList.Add().Text = message.BCC;
                                                ...
                                                out BodyPartInfo[] partList)
        {
            ...
            if (uid == string.Empty) {
                plainBody = "This message hasn't Unique Identfier. Attachments are not supported for such messages.";
                return;
            }
