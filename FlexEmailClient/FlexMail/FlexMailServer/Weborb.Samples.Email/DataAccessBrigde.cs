...
namespace Weborb.Samples.Email
{
    internal sealed class DataAccessBrigde
    {
        ...
        private const string OLEDB_DATA_ACCESS_LAYER = "OleDBDataSource";
        private const string MY_SQL_DATA_ACCESS_LAYER = "MySqlDataSource";
        ...
        private static string DEFAULT_CONNECTION_STRING =
            ...
            "Data Source=\"" + AppDomain.CurrentDomain.BaseDirectory + "\\email_client.mdb\";" +
            "User ID=Admin";
        ...
        internal const string SQL_INSERT_UNIQUE_ADDRESS =
            "insert into address (accountid, email) select top 1 {0}, '{1}' from address where not exists (select 1 from address where accountid = {0} and email = '{1}')";
        ...
        private static string DataAccessLayer {
        ...
        internal static int ExecuteNonQuery(IDbConnection connection, CommandType cmdType,
        ...
        internal static IDataReader ExecuteReader(IDbConnection connection, CommandType cmdType,
                                                  string cmdText, params DbParameter[] cmdParms) {
            IDbCommand cmd = CreateCommand();
            PrepareCommand(connection, cmd, cmdType, cmdText, cmdParms);
        ...
        internal static void PrepareCommand(IDbConnection connection, IDbCommand cmd,
