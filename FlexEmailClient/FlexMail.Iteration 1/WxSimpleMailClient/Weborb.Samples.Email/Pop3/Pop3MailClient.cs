...
namespace Weborb.Samples.Email.Pop3
{
    ...
    public class Pop3MailClient : IDisposable {
        ...
        public const string NEGATIVE_RESPONSE = "Negative server response for request ({0}). Response is {1}";
        ...
        public const string WRONG_RESPONSE = "Server return response in wrong format";
        ...
        protected void EnsureState(Pop3ConnectionStateEnum requiredState) {
            if (pop3ConnectionState != requiredState) {
                ...
                throw new Pop3Exception("GetMailboxStats only accepted during connection state: " +
                                        requiredState.ToString() +
                                        "\n The connection to server " + popServer + " is in state " +
                                        pop3ConnectionState.ToString());
            }
        }
        ...
        public void Connect() {
            if (pop3ConnectionState != Pop3ConnectionStateEnum.Disconnected &&
                ...
                !isTimeoutReconnect) {
                CallWarning("connect", "",
                            "Connect command received, but connection state is: " + pop3ConnectionState.ToString());
            } else {
                //establish TCP connection
                try {
                    CallTrace("   Connect at port {0}", port);
                    serverTcpConnection = new TcpClient(popServer, port);
                    serverTcpConnection.ReceiveTimeout = readTimeout;
                    serverTcpConnection.SendTimeout = readTimeout;
                } catch (Exception ex) {
                ...
                if (!executeCommand("PASS " + password, out response)) {
                    throw new Pop3Exception("Server " + popServer + " doesn't accept password for user '" + 
                                            username + "'.\nMessage: " + response);
                }
        ...
        public ArrayList GetEmailSizeList() {
            EnsureState(Pop3ConnectionStateEnum.Connected);
            ...
            if (executeCommand("LIST", out response)) {
                ...
                while (readMultiLine(out response)) {
                    int size;
                    if (int.TryParse(response.Split(' ')[1], out size)) {
                        result.Add(size);
                    } else {
                        CallWarning("GetEmailIdList", response, "first characters should be integer (EmailId)");
                        throw new Pop3Exception(WRONG_RESPONSE);
        ...
        /// <summary>
        /// Send TOP command to POP 3 server to fetch header for one particular message
        /// </summary>
        /// <param name="MessageNo">ID of message required</param>
        /// <returns>false: negative server respond</returns>
        protected void SendTopCommand(int MessageNo) {
            EnsureState(Pop3ConnectionStateEnum.Connected);
            
            string response;
            if (!executeCommand(string.Format("TOP {0} 0", MessageNo.ToString()), out response)) {
                CallWarning("GetEmailHeader", response, "negative response for email (ID: {0}) request", MessageNo);                
        ...
        private bool executeCommand(string command, out string response) {
            ...
            if (response == null) {
                isSupressThrow =
                    executeReconnect(new IOException("Timeout", new SocketException(10053)), command, commandBytes);
                if (isSupressThrow) {
                    //wait for response one more time
                    response = pop3StreamReader.ReadLine();
                }
        ...
        public void Dispose() {
            if (Pop3ConnectionState == Pop3ConnectionStateEnum.Connected) {
                Disconnect();
            }
        }
