...
// Creating header & detail
    try {
        ...
        m_composer = new DefaultComposer(composerDescriptor, 
            m_header, m_detail);
    } catch (wxString ex) {
...
//    menuEdit->Append(Detail_Undo, "&Undo\tCtrl-Z");
    ...
    menuProcess->Append(Detail_Send, "Send data");
...
void MyFrame::OnSave(wxCommandEvent& WXUNUSED(event))
{
    ...
    for (int i=0; i<numberRows; i++) {
        ...
        // writing row to file
        wxString rowStr;
        for (int m=0; m<descriptor->GetSize(); m++) {
...
void MyFrame::OnCopy(wxCommandEvent& WXUNUSED(event))
{
    ...
    if (grid->IsSelection())
    {
        ...
        if (wxTheClipboard->Open()) {
        	...
        	wxMessageBox("Can't open clipboard for writing data.", "Error");
        }
...
#if 0
    long TDcol = GetDetail()->GetDescriptor()->GetElementDescriptorIndex(wxString("Termination Date"));
    long ACcol = GetDetail()->GetDescriptor()->GetElementDescriptorIndex(wxString("Action Code"));

    if ( col == ACcol || col == TDcol ) {
    	// todo: reaction on input Action Code or Termination Date
    	wxString val = wxString(GetDetail()->GetValue(row, ACcol)); 
    	if ( val.Cmp(" 1")==0 || val.Cmp(" 7")==0 ) {
    		if ( !IsCellValid(row, TDcol, GetDetail()->GetColumnDescriptor(TDcol)) ) {
    			grid->SetCellBackgroundColour(row, TDcol, NOT_VALID_COL);
    		} else {
    			grid->SetCellBackgroundColour(row, TDcol, REQUIRED_COL);
    		}
    	} else {
    		if ( !GetDetail()->GetValue(row, TDcol).IsEmpty() ) {
    			grid->SetCellBackgroundColour(row, TDcol, NOT_VALID_COL);
    			wxMessageBox("TerminationDate cannot be enterred, if ActionCode is not < 1> or < 7>", "Info");
    		} else {
    			grid->SetCellBackgroundColour(row, TDcol, VALID_COL);
    		}
    	}
    }
#else
...
bool MyFrame::IsCellValid(int i, int j, ElementDescriptor* ed)
{
    ...
    if ( ed->GetType().Cmp("date")==0 && GetDetail()->GetValue(i, j).Cmp("    /  /  ")==0 ) {
...
void MyFrame::RecreateToolbar()
{
    ...
    toolBar->AddTool(TOOL_ID_NEW, toolBarBitmaps[0], wxNullBitmap, FALSE, currentX, -1, (wxObject *) NULL, "New file");
    currentX += width + 5;
...
//    toolBar->AddSeparator();
    ...
    toolBar->AddTool(TOOL_ID_SEND, toolBarBitmaps[13], wxNullBitmap, FALSE, currentX, -1, (wxObject *) NULL, "Send EDI file");
    currentX += width + 5;
    ...
    toolBar->AddTool(TOOL_ID_HELP, toolBarBitmaps[7], wxNullBitmap, FALSE, currentX, -1, (wxObject *) NULL, "About");
...
void MyFrame::OnToolEnter(wxCommandEvent& event)
{
    if (event.GetSelection() > -1)
    {
        ...
        if (event.GetSelection() == TOOL_ID_SEND) { 
            str.Printf(_T(" tool - Send EDI file"));
        }
