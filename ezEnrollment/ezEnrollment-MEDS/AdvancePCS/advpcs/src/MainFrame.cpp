...
{
    if ( ev.Selecting() ) {
        SetStatusText(GetDocument()->GetColHint(ev.GetRow(), m_table->Map(ev.GetCol())), 1);
    }
    ev.Skip();
}
...
void MainFrame::OnCellValueChanged(wxGridEvent& ev) {
    ...
    if ( GetDocument()->IsValidValue(row, m_table->Map(col), false) ) {
        if ( GetDocument()->IsColRequired(row, m_table->Map(col)) ) {
...
void MainFrame::OnPaste(wxCommandEvent& event)
{
    ...
    try {
        ...
        if ( m_grid->IsSelection() ) {
			...
			int old_cursor_row = m_grid->GetGridCursorRow();
            m_commandProcessor.Submit(new CmdPaste(*m_grid, *m_table, matrix, 
			...
			for ( int i = old_cursor_row; i < old_cursor_row + matrix.size(); i++ ) {
				//ValidateRow(i);
			}
		}
        UpdateToolbar();
...
void MainFrame::UpdateRow(int row, bool refresh) {

	for ( int col = 0; col < m_table->GetNumberCols(); col++ ) {
	    if ( GetDocument()->IsColRequired(row, m_table->Map(col)) ) {
			if ( VALID_COL == m_grid->GetCellBackgroundColour(row, col) ) {
				m_grid->SetCellBackgroundColour(row, col, REQUIRED_COL);
			}
        } else {
			if ( REQUIRED_COL == m_grid->GetCellBackgroundColour(row, col) ) {
	            m_grid->SetCellBackgroundColour(row, col, VALID_COL);
			}
        }
	}
...
bool MainFrame::ValidateRow(int row) {
	for ( int col = 0; col < m_table->GetNumberCols(); col++ ) {
...
void MainFrame::UpdateToolbar() {
	wxMenu *menuEdit = GetMenuBar()->GetMenu(GetMenuBar()->FindMenu("Edit"));
	wxMenu *menuProcess = GetMenuBar()->GetMenu(GetMenuBar()->FindMenu("Process"));
#if 1
    if ( m_commandProcessor.CanUndo() ) {
        m_toolBar->EnableTool(TOOL_ID_UNDO, true);
		menuEdit->Enable(menuEdit->FindItem("Undo"), true);
    } else {
        m_toolBar->EnableTool(TOOL_ID_UNDO, false);
		menuEdit->Enable(menuEdit->FindItem("Undo"), false);
    }
    if ( m_commandProcessor.CanRedo() ) {
        m_toolBar->EnableTool(TOOL_ID_REDO, true);
		menuEdit->Enable(menuEdit->FindItem("Redo"), true);
    } else {
        m_toolBar->EnableTool(TOOL_ID_REDO, false);
		menuEdit->Enable(menuEdit->FindItem("Redo"), false);
    }
	if ( ::wxGetApp().GetAgent().IsEnabled() ) {
		m_toolBar->EnableTool(TOOL_ID_SEND, true);
		menuProcess->Enable(menuProcess->FindItem("Send data to server"), true);
	} else {
		m_toolBar->EnableTool(TOOL_ID_SEND, false);
		menuProcess->Enable(menuProcess->FindItem("Send data to server"), false);
	}
#endif
};
