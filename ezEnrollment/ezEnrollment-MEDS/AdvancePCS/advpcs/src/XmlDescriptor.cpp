...
static wxString CHECK_VALUE_FIELD_ATTR(wxT( "check_field"));
static wxString CHECK_VALUE_VALUE_ATTR(wxT( "check_value"));
static wxString CHECK_VALUE_THEN_ATTR(wxT(  "check_then" ));
...
static wxString TRAN_NAME_ATTR (wxT( "tran_name"  ));
static wxString TRAN_REQUIRED_ATTR (wxT( "tran_required"  ));
...
//						return "Date is earlier than start_date.  date: " + date.Format("%m/%d/%Y") + " s_date: " + s_date.Format("%m/%d/%Y");
    ...
    } else if ( GetType().Cmp("regexp")==0 && (!value.IsEmpty()) ) {
...
const wxString& XmlFieldDescriptor::GetCheckValueField() const {
    return m_check_value_field;
};

const wxArrayString& XmlFieldDescriptor::GetCheckValueValues() const {
    return m_check_value_values;
};

const wxString& XmlFieldDescriptor::GetCheckValueThenPattern() const {
    return m_check_value_then;
};

wxString XmlFieldDescriptor::GetTranName() const {
	return m_tran_name;
};

wxArrayString XmlFieldDescriptor::GetTranRequired() const {
	return m_tran_required;
};
...
bool XmlFieldDescriptor::MatchPattern(const wxString& candidate, const wxString& pattern) const {
    ...
    if ( pattern.IsEmpty() ) {
        ...
        if ( '!' == designator || '>' == designator || '<' == designator) {
...
// transaction field name, transaction required

    if ( GetNode().HasProp(TRAN_NAME_ATTR.c_str()) ) {
        m_tran_name = GetNode().GetPropVal(TRAN_NAME_ATTR.c_str(), "");
	} else {
		m_tran_name = wxEmptyString;
	}
	
    if ( GetNode().HasProp(TRAN_REQUIRED_ATTR.c_str()) ) {
		if ( 0 == m_tran_name.Cmp(wxEmptyString) ) {
	        THROW_CFG_EXCEPTION(GetName() + ": [tran_name] can not be empty. " + ADVPCS_DESC_WRONG_XML);
		}
        wxString required_str = GetNode().GetPropVal(TRAN_REQUIRED_ATTR.c_str(), "");
		wxStringTokenizer st( required_str, _T(";") );
		while ( st.HasMoreTokens() ) {
			m_tran_required.Add(st.GetNextToken());
		}
	}
...
//--------
    ...
    if ( GetNode().HasProp(DEFAULT_ATTR.c_str()) ) {
        ...
        if ( (0 == m_type.CmpNoCase("date") || 
			  0 == m_type.CmpNoCase("date0") || 
			  0 == m_type.CmpNoCase("date9") ||
			  0 == m_type.CmpNoCase("date09") ||
			  0 == m_type.CmpNoCase("longdate") ||
			  0 == m_type.CmpNoCase("longdate0") ||
			  0 == m_type.CmpNoCase("longdate9") ||
			  0 == m_type.CmpNoCase("longdate09") ) && 0 == m_defaultValue.CmpNoCase("now")) { 
    ...
    if ( GetNode().HasProp(CHECK_VALUE_FIELD_ATTR.c_str()) ) {
        m_check_value_field = GetNode().GetPropVal(CHECK_VALUE_FIELD_ATTR.c_str(), "");

        if ( GetNode().HasProp(CHECK_VALUE_VALUE_ATTR.c_str()) ) {
			wxString v_str = GetNode().GetPropVal(CHECK_VALUE_VALUE_ATTR.c_str(), "");
			wxStringTokenizer st( v_str, _T(";") );
			while ( st.HasMoreTokens() ) {
				m_check_value_values.Add(st.GetNextToken());
			}
        } else {
            THROW_CFG_EXCEPTION(GetName() + ": " + ADVPCS_DESC_WRONG_XML);
        }

        if ( GetNode().HasProp(CHECK_VALUE_THEN_ATTR.c_str()) ) {
            m_check_value_then = GetNode().GetPropVal(CHECK_VALUE_THEN_ATTR.c_str(), "");
        } else {
            THROW_CFG_EXCEPTION(GetName() + ": " + ADVPCS_DESC_WRONG_XML);
        }

    } else {
        m_check_value_field = wxEmptyString;
    }
