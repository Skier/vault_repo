...
bool AdvPCSApp::OnInit() {
    ...
    try {
        ...
        m_agent = new SftpAgent(*(m_agentCfg->GetChild(ADVPCS_AGENT_CFG)), GetLogger(), m_frame->GetProcessIndicator());
...
unsigned long AdvPCSApp::EdiFileCounter() {
	unsigned long counter;
    wxTextFile file(ADVPCS_COUNTER_FILE_NAME);
    if ( !file.Open() ) {
		counter = 1;
    } else {
		wxString countStr = file.GetLine(0);
		if ( !countStr.ToULong(&counter) ) {
			counter = 1;
		}
		file.Close();
	}

    wxFile out(ADVPCS_COUNTER_FILE_NAME, wxFile::write);

    if ( !out.IsOpened() ) {
        THROW_SYSTEM_EXCEPTION(ADVPCS_COUNTER_FILE_NAME);
    }
	
	wxString new_counter;
	new_counter << counter + 1;
    out.Write(new_counter + "\r\n");
	out.Close();
