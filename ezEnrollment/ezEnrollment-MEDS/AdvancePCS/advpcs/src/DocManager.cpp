...
DocManager::DocManager(const CXmlNode& cfg, ProcessIndicator& indicator) 
    : m_headerDesc(NULL), m_detailDesc(NULL), m_trailerDesc(NULL) 
{
...
DocManager::~DocManager() {
    ...
    if ( NULL != m_trailerDesc ) {
        wxDELETE(m_trailerDesc);
    }
};
...
Document* DocManager::Create() const {
    ...
    return new EdiDocument(*m_headerDesc, *m_detailDesc, *m_trailerDesc);
};
...
void DocManager::BuildDescriptors(const CXmlNode& cfg) {
    ...
    CXmlNode* trailerNode = cfg.GetChild(ADVPCS_TRAILER_CFG.c_str());
    if ( NULL == trailerNode ) {
        THROW_CFG_EXCEPTION(ADVPCS_CFG_TRAILER_NOT_FOUND);
    }
    m_trailerDesc = new XmlDescriptor(*trailerNode);
	m_trailerDesc->CheckValid();
