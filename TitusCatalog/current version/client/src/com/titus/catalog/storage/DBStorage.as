	...
	implements IStorage
{
	...
	public function getCatalogItemPackage(catalogItem:CatalogItem):CatalogItemPackage
	{
		...
		if (4 == catalogItem.CatalogLevel) {
        	...
        	service.GetSubmittals.addEventListener(
        		ResultEvent.RESULT,
        		function(evt:ResultEvent):void {
        			for each (var submittalInfo:Submittal in (evt.result as Array)) {
        				result.submittals.addItem(submittalInfo);
        				
        				for each (var cartSubmittalInfo:Submittal in cart) {
        					if (cartSubmittalInfo.FileId == submittalInfo.FileId) {
        						submittalInfo.isInCart = true;
        						break;
        					}
        				}
        			}
        	...
        	service.GetSubmittals(catalogItem.ModelId);
		}
