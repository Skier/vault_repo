...
namespace Titus.ECatalog.Service
{
        ...
        public class UploaderService : IHttpHandler
        {
            ...
            private DocumentDataObject ParseCatalog(SqlTransaction tran, int id)
            {
                DocumentDataObject documentInfo = Document.GetInstance().FindById(tran, id);
                ...
                using (XmlTextReader reader = new XmlTextReader(stream))
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            ParseDocument(reader, tran, documentInfo);
                        }
                    }
                }

                Document.GetInstance().Update(tran, documentInfo);
            ...
            private void ParseDocument(XmlTextReader reader, SqlTransaction tran, DocumentDataObject catalogInfo)
            {
                int pageCounter = 0;
                ...
                while (reader.Read())
                {
                    ...
                    else if ("PAGE" == reader.Name.ToUpper())
                    {
                        ...
                        pageInfo.PageNumber = pageCounter;
                        pageInfo.ScreenshotURL =
                            Path.GetDirectoryName(catalogInfo.Path)
                            + "\\" + catalogInfo.DocumentId.ToString()
                            + "\\" + pageCounter.ToString("-000000") + ".png";
                        pageInfo.DocumentId = catalogInfo.DocumentId;
            ...
            private void ParseMetadata(XmlTextReader reader, DocumentDataObject catalogInfo)
            {
                while (reader.Read())
                {
                    if (XmlNodeType.Element != reader.NodeType)
                    ...
                    if ("PDFFILENAME" == reader.Name.ToUpper())
                    {
                        ...
                        catalogInfo.Name = reader.ReadContentAsString();
            ...
            private void ParsePage(XmlTextReader reader, SqlTransaction tran, PageDataObject pageInfo)
            {
                pageInfo.Width = Int32.Parse(reader.GetAttribute("width"));
                pageInfo.Height = Int32.Parse(reader.GetAttribute("height"));

                Page.GetInstance().Insert(tran, pageInfo);
            ...
            private void ParseText(XmlTextReader reader, SqlTransaction tran, PageDataObject pageInfo)
            {
                while (reader.Read())
                {
                    if (XmlNodeType.Element == reader.NodeType)
                    {
                        if ("TOKEN" == reader.Name.ToUpper())
                        {
                            ...
                            tokenInfo.DocumentPageId = pageInfo.DocumentPageId;
