  ...
  <xsl:template name="files">
    ...
    <file name="codegen\{@name}.Generator.cs" type="store" overwrite="true">
      ...
      <xsl:value-of select="codegen:Progress(concat('Generating business class from ', @name))" />
        ...
        #region Fields
        <xsl:for-each select="xs:complexType/xs:attribute">
          protected <xsl:value-of select="codegen:CSharpDataType(@type)"/><xsl:if test="@use = 'optional' and (@type='xs:int' or @type='xs:date')">?</xsl:if><xsl:text> </xsl:text>m_<xsl:value-of select="codegen:FunctionParameter(@name)" />;
        </xsl:for-each>
        #endregion
        ...
        public <xsl:value-of select="$biz-name"/>(
        <xsl:for-each select="xs:complexType/xs:attribute">
          <xsl:value-of select="codegen:CSharpDataType(@type)" /><xsl:if test="@use = 'optional' and (@type='xs:int' or @type='xs:date')">?</xsl:if>
          <xsl:text> 
          </xsl:text>
          <xsl:value-of select="codegen:FunctionParameter(@name)" />
          <xsl:if test="position() != last()">,</xsl:if>
      ...
      <xsl:for-each select="xs:complexType/xs:attribute">
        [XmlElement]
        public <xsl:value-of select="codegen:CSharpDataType(@type)" /><xsl:if test="@use = 'optional' and (@type='xs:int' or @type='xs:date')">?</xsl:if><xsl:text> </xsl:text><xsl:value-of select="@name" />
        {
