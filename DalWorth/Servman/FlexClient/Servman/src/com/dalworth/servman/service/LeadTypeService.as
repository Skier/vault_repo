package com.dalworth.servman.service
{
	...
	public class LeadTypeService extends EventDispatcher
	{
		private static var _instance:LeadTypeService;
		...
		public function LeadTypeService()
		{
			super(null);
		...
		public static function getInstance():LeadTypeService 
		{
			if (_instance == null)
				_instance = new LeadTypeService();
			
			return _instance;
		}
		
		public function getLeadType(id:int):AsyncToken 
		{
			...
			asyncToken.addResponder(
				new Responder(
					...
					{
						LeadTypeRegistry.getInstance().storeLocal(event.result);
					},
		...
		public function getAll():AsyncToken 
		{
			...
			asyncToken.addResponder(
				new Responder(
					function (event:ResultEvent):void
					{
						LeadTypeRegistry.getInstance().forceUpdate(event.result as Array);
					},
					function (event:FaultEvent):void
					{
						trace(event.fault.message);
		...
		public function saveLeadType(value:LeadType):AsyncToken 
		{
		...
		private function get service():RemoteObject
		{
			if (_service == null) {
				...
				_service.source = "Servman.Intuit.Weborb.LeadTypeService";
