package com.affilia.voice 
{
    ...
    public class SoundFacade extends EventDispatcher
    {
         ...
         */
        public var progressInterval:int = 200;
        ...
        {   
            trace("loadingProgress - " + event.bytesLoaded + ":" + event.bytesTotal);
            this.dispatchEvent(event.clone());
        }
        ...
        public function seek(pct:Number):void
        {
        	...
        	var position:Number = s.length * pct;
        ...
        {
            trace("lenght: " + s.length + ", loaded:" + s.bytesLoaded + ", total:" + s.bytesTotal);

            var estimatedLength:int;
            if (isLoaded)
                estimatedLength = Math.ceil(this.s.length);
            else 
                estimatedLength = Math.ceil(this.s.length / (this.s.bytesLoaded / this.s.bytesTotal));

            var progEvent:ProgressEvent = 
                new ProgressEvent(PLAY_PROGRESS, false, false, this.sc.position, estimatedLength);
            
            trace("playProgress - " + this.sc.position + ":" + estimatedLength);
