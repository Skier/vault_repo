...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="/">
    <codegen>
      ...
      <file name="as\IDomainEntity.as" overwrite="true">
...
namespace Dalworth.LeadCentral.Domain
{
    ...
    <xsl:if test="codegen:IsGenActionScriptClass()">

      <file name="as\{$biz-name}.as" overwrite="true">
package com.dalworth.leadCentral.domain
{
    import com.dalworth.leadCentral.domain.codegen.*;

    [Bindable]
    [RemoteClass(alias="Servman.Domain.<xsl:value-of select="$biz-name"/>")]
    public class <xsl:value-of select="$biz-name"/> extends _<xsl:value-of select="$biz-name"/>
    {
        public function <xsl:value-of select="$biz-name"/>()
        {
        }
    }
}
      ...
      <file name="as\codegen\_{$biz-name}.as" overwrite="false">
package com.dalworth.leadCentral.domain.codegen
{
    import com.dalworth.leadCentral.domain.*;
    
    [Bindable]
    public class _<xsl:value-of select="$biz-name"/> implements IDomainEntity
    {
        public function _<xsl:value-of select="$biz-name"/>()
        {
        }
      <xsl:for-each select="xs:complexType/xs:attribute">
        private var <xsl:value-of select="codegen:ActionScriptPrivateProperty(@name)" /><xsl:text>:</xsl:text><xsl:value-of select="codegen:ActionScriptDataType(@type)" /><xsl:text>;</xsl:text>
        public function get <xsl:value-of select="codegen:ActionScriptProperty(@name)" />():<xsl:value-of select="codegen:ActionScriptDataType(@type)" /> { return <xsl:value-of select="codegen:ActionScriptPrivateProperty(@name)" />; }
        public function set <xsl:value-of select="codegen:ActionScriptProperty(@name)" />(value:<xsl:value-of select="codegen:ActionScriptDataType(@type)" />):void 
        {
            <xsl:value-of select="codegen:ActionScriptPrivateProperty(@name)" /> = value;
        }
      </xsl:for-each>

        public function prepareToSend():<xsl:value-of select="$biz-name"/>
        {
            var result:<xsl:value-of select="$biz-name"/> = new <xsl:value-of select="$biz-name"/>();
      <xsl:for-each select="xs:complexType/xs:attribute">
            result.<xsl:value-of select="codegen:ActionScriptProperty(@name)" /> = this.<xsl:value-of select="codegen:ActionScriptProperty(@name)" />;
      </xsl:for-each>
            return result;
        }
        ...
        {
            if (value == null)
                value = new <xsl:value-of select="$biz-name"/>();
      <xsl:for-each select="xs:complexType/xs:attribute">
            this.<xsl:value-of select="codegen:ActionScriptProperty(@name)" /> = value["<xsl:value-of select="codegen:ActionScriptProperty(@name)" />"];
      </xsl:for-each>
        }
    }
}
