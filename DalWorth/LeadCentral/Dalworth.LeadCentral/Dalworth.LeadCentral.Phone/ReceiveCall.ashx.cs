...
";
        ...
        {
            return BillingService.GetCurrentBalance(servmanCustomer) <= 0;
        }
        ...
        private static Lead CreateLead(PhoneCall phoneCall, ServmanCustomer servmanCustomer)
        {
            ...
            return LeadService.Save(servmanCustomer, lead);
        }

        private static int? GetLeadSourceId(PhoneCall phoneCall, ServmanCustomer servmanCustomer)
        {
            List<LeadSource> leadSources =
                LeadSourceService.GetByPhoneNumber(servmanCustomer, phoneCall.PhoneFrom);
            if (leadSources.Count == 1 && leadSources[0].IsActive)
                return leadSources[0].Id;

            leadSources = LeadSourceService.GetByTrackingPhoneNumber(servmanCustomer, phoneCall.TrackingPhoneId);
            if (leadSources.Count == 1 && leadSources[0].IsActive)
                return leadSources[0].Id;
        ...
        private static void SyncWithPhoneRotation(PhoneCall phoneCall, ServmanCustomer servmanCustomer)
        {
            ...
            TrackingPhoneRotation trackingPhoneRotation = PhoneService.GetLastPhoneRotationByPhoneId(servmanCustomer, phoneCall.TrackingPhoneId);
