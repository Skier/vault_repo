...
namespace Intuit.Platform.Client.OAuth.Server
{
	...
	public class OAuthConnector : IOAuthConnector
	{
        ...
        public OAuthConnector(IOAuthUrls urls, IOAuthKeyStore keyStore, AsymmetricAlgorithm key)
        {
            m_Urls = urls;
            m_KeyStore = keyStore;
            m_Key = key;
		...
		{
            IOAuthConsumerContext consumerContext = null;

            if (m_Key == null)
            {
                consumerContext = CreateConsumerContext(true);
            }
            else
            {
                consumerContext = CreateConsumerContext(true, m_Key);
            }

			IOAuthSession oAuthSession = CreateOAuthSessionWithConsumerContext(consumerContext);
			oAuthSession.AccessToken = CreateAccessToken();
            String oAuthHeader = oAuthHeader = GetOAuthHeaderForRequest(oAuthSession, xrequest, bodyIfFormEncodedParams);

			xrequest.Headers.Add("Authorization", oAuthHeader);
		}

        public void Authorize(WebRequest xrequest, string bodyIfFormEncodedParams, NameValueCollection additionalParams)
        {
            IOAuthConsumerContext consumerContext = null;
            ...
            IOAuthSession oAuthSession = CreateOAuthSessionWithConsumerContext(consumerContext);
            oAuthSession.AccessToken = CreateAccessToken();
            string oAuthHeader = GetOAuthHeaderForRequest(oAuthSession, xrequest, bodyIfFormEncodedParams, additionalParams);
            xrequest.Headers.Add("Authorization", oAuthHeader);
        }
        ...
        public static string GetOAuthHeaderForRequest(IOAuthSession oAuthSession, WebRequest webRequest, string bodyIfFormEncodedParams, NameValueCollection additionalParams)
        {
            IConsumerRequest consumerRequest = oAuthSession.Request();
            consumerRequest = ConsumerRequestExtensions.ForMethod(consumerRequest, webRequest.Method);
            consumerRequest = ConsumerRequestExtensions.ForUri(consumerRequest, webRequest.RequestUri);
            ...
            {
                consumerRequest.Context.AuthorizationHeaderParameters.Add(additionalParams);
            }
        ...
        public IOAuthConsumerContext CreateConsumerContext(bool useHeader, System.Security.Cryptography.AsymmetricAlgorithm key)
        {
            IOAuthConsumerContext consumerContext = new OAuthConsumerContext
            {
                ConsumerKey = m_KeyStore.ConsumerKey,
