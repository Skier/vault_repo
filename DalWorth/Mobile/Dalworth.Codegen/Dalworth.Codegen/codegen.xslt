...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template name="files">
    ...
    <file name="codegen\{@name}.Generator.cs" type="store" overwrite="true">
      ...
      public partial class <xsl:value-of select="@name"/> : DomainObject
      {
      ...
      public static void Insert(<xsl:value-of select="$biz-name" /><xsl:text> </xsl:text><xsl:value-of select="$functionParam" />, IDbTransaction transaction)
      {
      ...
      public static void Insert(List&lt;<xsl:value-of select="$biz-name" />&gt; <xsl:text> </xsl:text><xsl:value-of select="$functionParam" />List, IDbTransaction transaction)
      {
      using(IDbCommand dbCommand = Database.PrepareCommand(SqlInsert, transaction))
      {
      ...
      public static void Update(<xsl:value-of select="$biz-name" /><xsl:text> </xsl:text><xsl:value-of select="$functionParam" />, IDbTransaction transaction)
      {
      ...
      </xsl:for-each>, IDbTransaction transaction
      )
      {
      ...
      public static bool Exists(<xsl:value-of select="$biz-name" /> <xsl:text> </xsl:text><xsl:value-of select="$functionParam" />, IDbTransaction transaction)
      {
      ...
      ;
      public static void Delete(<xsl:value-of select="$biz-name" /><xsl:text> </xsl:text><xsl:value-of select="$functionParam" />, IDbTransaction transaction)
      {
      ...
      + " From [<xsl:value-of select="$table" />] ";
      public static List&lt;<xsl:value-of select="$biz-name" />&gt; Find(IDbTransaction transaction)
      {
