...
namespace Dalworth.Data
{
	...
	public static class Database
	{
        ...
        {
            IDbCommand dbCommand = PrepareCommand(SqlIsSchemaExist);
        ...
        public static void ExecuteScript(String filePath)
        {
            ...
            if (!File.Exists(filePath))
                throw new DalworthException("Stript file " + filePath + " not found");
            ...
            catch (Exception ex)
            {
                ...
                throw new DalworthException(sqlBuffer.ToString(), ex); 
            }
        }
        ...
        {
            if (IsDatabaseExist())
                throw new DalworthException("Database already exists");
            ...
            if (createSchema)
            {
                try
                {                    
                    string path = Path.GetDirectoryName(Configuration.AppNameFullPath);
		...
		public static IDbCommand PrepareCommand(String sql, IDbTransaction transaction)
		{
            if (transaction == null)
                return PrepareCommand(sql);
            else
                return PrepareCommand(sql, transaction.Connection, transaction);
		}
