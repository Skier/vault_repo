...
namespace Dpi.Central.Web.Account.Payment
{
    public class CreditCardInfoControl : UserControl
    {
        ...
        {
            if (lstExpMonth.Items.Count == 0) {
                lstExpMonth.Items.Add(string.Empty);                                
                
                for (int i = 1; i <= 12; i++) {
                    lstExpMonth.Items.Add(new ListItem(months[i], i.ToString()));
                }
            }

            if (lstExpYear.Items.Count == 0) {
                lstExpYear.Items.Add(string.Empty);
        ...
        public bool Enabled
        {
            set
            {
                ...
                vldCstCcNumber.Enabled = vldCstCreditCardType.Enabled = vldCstExpDate.Enabled = vldReCcNumber.Enabled = vldRfCcNumber.Enabled = vldRfCvNumber.Enabled = value;
