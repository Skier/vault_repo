...
phone: 1-800-350-4009";
            ...
            out PaymentResult paymentResult) 
        {
            ...
            try {
                ...
                if (paymentResult.Code != PaymentResultCode.Completed)
                    throw new PaymentException(paymentResult);                                    
        ...
        public static EnableWebAccessStatus EnableWirelessWebAccess(IMap imap, string phoneNumber, string accLastName, string email, string webPassword) 
        {
            EnableWebAccessStatus result = EnableWebAccessStatus.Success;
            ...
            try {
                ...
                if (custInfoList[0].NameLast == null || (
                    custInfoList[0].NameLast.Trim().ToUpper() != accLastName.Trim().ToUpper())) {
                    result = EnableWebAccessStatus.LastNameDoesNotMatch;
                    return result;
                }

                if (email.Length > 0) {
                    custInfoList[0].Email = email;
                }

                custInfoList[0].WebPassword = webPassword;
