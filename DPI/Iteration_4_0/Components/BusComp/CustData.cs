...
namespace DPI.Components
{
    ...
    public class CustData : DomainObj, IId, ICustDataValidation
    {
        ...
        string verbatumServiceAddress;
        string verbatumMailingAddress;
		...
		public string WebPassword
		{
			get
			{
                ...
                GetWebPasswordParts(out password, out temporal);
			...
			set 
			{	setState();
                SetWebPasswordParts(value, IsWebPasswordTemporal);
			}
		}
        ...
        public bool IsWebPasswordTemporal
        {
            ...
            {
                setState();
                SetWebPasswordParts(WebPassword, value);
            }
        }
        ...
        {
            get { return verbatumServiceAddress; }
            set
            {
                setState();
                verbatumServiceAddress = value;
            }
        }

        public string VerbatumMailingAddress
        {
            get { return verbatumMailingAddress; }
            set
            {
                setState();
                verbatumMailingAddress = value;
            }
        }
...
//		}
        ...
        public static void copyAttrs(CustData src, CustData tar)
        {
            ...
            tar.additionalInfo = src.additionalInfo;
            tar.verbatumServiceAddress = src.verbatumServiceAddress;
            tar.verbatumMailingAddress = src.verbatumMailingAddress;

            tar.rowState = src.rowState;
        }
 
        private void GetWebPasswordParts(out string password, out bool temporal) 
        {
            password = null;
            temporal = false;
            ...
            string flag = webPassword.Substring(0, 1);
            if (flag != "0" && flag != "1") {
                throw new DataException("WebPassword field must be either NULL or " 
                    + "begin with '0' or '1'.");
            }

            password = (webPassword.Length == 1 ? string.Empty
                : webPassword.Substring(1, webPassword.Length - 1));
            temporal = flag == "1";
        }

        private void SetWebPasswordParts(string password, bool temporal) 
        {
            string temporalPart = temporal ? "1" : "0";

            if (password == null || password.Length == 0) {
                webPassword = temporalPart;
            } else {
                webPassword = temporalPart + password;
            }
        }
 
        /*		SQL		*/
        [Serializable]
        protected class CustDataSQL : SqlGateway
        {
            ...
            void setParam(SqlCommand cmd, CustData rec)
            {
                ...
                if (rec.language == null) 
                    cmd.Parameters.Add("@Language", SqlDbType.VarChar, 10).Value = "English";
                else
				...
				if (rec.webPassword == null || rec.webPassword.Length == 0)
					cmd.Parameters.Add("@WebPassword", SqlDbType.VarChar, 25).Value = DBNull.Value;
				else
                    cmd.Parameters.Add("@WebPassword", SqlDbType.VarChar, 25).Value = rec.webPassword;

                if (rec.additionalInfo == null)
                    cmd.Parameters.Add("@AdditionalInfo", SqlDbType.VarChar, 100).Value = DBNull.Value;
                else {
                    if (rec.additionalInfo.Length == 0)
                        cmd.Parameters.Add("@AdditionalInfo", SqlDbType.VarChar, 100).Value = DBNull.Value;
                    else
                        cmd.Parameters.Add("@AdditionalInfo", SqlDbType.VarChar, 100).Value = rec.additionalInfo;
                }

                if (rec.verbatumServiceAddress == null) 
                    cmd.Parameters.Add("@VerbatumServiceAddress", SqlDbType.VarChar, 200).Value = DBNull.Value;
                else
                    cmd.Parameters.Add("@VerbatumServiceAddress", SqlDbType.VarChar, 200).Value = rec.verbatumServiceAddress;

                if (rec.verbatumMailingAddress == null) 
                    cmd.Parameters.Add("@VerbatumMailingAddress", SqlDbType.VarChar, 200).Value = DBNull.Value;
                else
                    cmd.Parameters.Add("@VerbatumMailingAddress", SqlDbType.VarChar, 200).Value = rec.verbatumMailingAddress;
            }

            protected virtual void loadCustData(SqlDataReader rdr, CustData rec)
            {
                if (rdr["AccNumber"] != DBNull.Value)
                ...
                if (rdr["WebPassword"] != DBNull.Value)
                    rec.webPassword = (string)rdr["WebPassword"];

                if (rdr["AdditionalInfo"] != DBNull.Value)
                    rec.additionalInfo = (string)rdr["AdditionalInfo"];

                if (rdr["VerbatumServiceAddress"] != DBNull.Value)
                    rec.verbatumServiceAddress = (string)rdr["VerbatumServiceAddress"];

                if (rdr["VerbatumMailingAddress"] != DBNull.Value)
                    rec.verbatumMailingAddress = (string)rdr["VerbatumMailingAddress"];
