	...
	public class Wireless_Transactions : DomainObj, IWireless_Transactions, IPayInfoTran
	{
		...
		public IWireless_Custdata Customer
		{
			get 
			{
				if (customer == null)
					if (acctID > 0)
						customer = Wireless_Custdata.find(uow, acctID);
 
				return customer;
			}
			set 
			{ 
				customer = (IWireless_Custdata)value; 
				acctID = 0;
		...
		public static int PostPendWirelessTran(UOW uow, 
			...
			string clerkId,
			string pin, 
			decimal webProdPrice,
			string trNumber)
		{
			return new Wireless_TransactionsSQL().PostPendTran(uow, prod, storeCode, clerkId, pin, webProdPrice, trNumber);
			
		}
		public static int SetStatusById(UOW uow, int id, string trNumber, string status)
		{
			return new Wireless_TransactionsSQL().SetStatusById(uow, id, trNumber, status);			
		}
			...
			class Wireless_TransactionsSQL : SqlGateway
		{
			...
			public int PostPendTran(UOW uow, int prod, string storeCode, string clerkId, string pin, decimal webProdPrice, string trNumber)
			{
				SqlCommand cmd = makeCommand(uow);
				cmd.CommandText = "spWS_createPendWirelessTransactionWebCentral";
				
				setPostTranParam(cmd, prod, storeCode, clerkId, pin, webProdPrice, trNumber);
				return execScalarInt(cmd);
			}
			public int SetStatusById(UOW uow, int id, string trNumber, string status)
			{
				SqlCommand cmd = makeCommand(uow);
				cmd.CommandText = "spWireless_Transactions_SetStatusById";
				
				setSetStatusByIdParam(cmd, id, trNumber, status);
				return execScalarInt(cmd);
			}
			...
			{
				SqlCommand cmd = makeCommand(uow);
				cmd.CommandText = "spWireless_Transactions_Get_PendWirelessByStore";
				cmd.Parameters.Add("@StoreCode", SqlDbType.Char, 10).Value = storeCode;
			...
			void setPostTranParam(SqlCommand cmd, int prod, string storeCode, string clerkId, string pin, decimal webProdPrice, string trNumber)
			{
				...
				cmd.Parameters.Add("@WebProdPrice", SqlDbType.Decimal, 0).Value = webProdPrice;
				if (trNumber == null)
			...
			void setSetStatusByIdParam(SqlCommand cmd, int id, string trNumber, string status)
			{
				cmd.Parameters.Add("@Wireless_Transaction_ID", SqlDbType.Int, 0).Value = id;
