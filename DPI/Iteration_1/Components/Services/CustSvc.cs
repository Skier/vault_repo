...
namespace DPI.Services
{
    ...
    public class CustSvc
    {
            ...
            IMap imap, string phNumber, string webPassword)
        {
            ...
            try {
                ...
                if (custArr.Length == 0) {
                    ...
                    return result;
                } else if (custArr[0].WebPassword != webPassword.Trim()) {
                    result.Status = WebValidationResultStatus.InvalidCustomer;
                    return result;
                } else {
        ...
        public static void EnableWebAccess(
            IMap imap, int accNumber, string accLastName, string email, string webPassword)
        {
            if (imap == null) {
                throw new ArgumentException("imap is required");
            }
            ...
            try {
                ...
                // bad data
                if (custData.SourceCode == 0) {
                    custData.SourceCode = 1;
                }
            ...
            try {
                ...
                if (custData != null) {
                    if (custData.WebPassword != null && custData.WebPassword.Length > 0) {
                        ...
                        msg.EmailFrom = "agent.inquiry@dpiteleconnect.com";
                        msg.EmailSubject = "dPi Website Password Reminder";
                        msg.EmailMessage = "Your password is " + custData.WebPassword;
        ...
        public static IWireless_Custdata GetWirelessCustData(IMap imap, int id) {
            ...
            try {
                ...
                return (IWireless_Custdata)Wireless_Custdata.find(uow, id);
            }
            finally {
                uow.close();
                imap.ClearDomainObjs();
