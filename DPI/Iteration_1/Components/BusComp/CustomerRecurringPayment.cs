...
{
	[Serializable]
	public class CustomerRecurringPayment : DomainObj, ICustomerRecurringPayment
	{
		#region Data
		static string iName = "CustomerRecurringPayment";
		int id;
		...
		#region Constructors
		public CustomerRecurringPayment()
		{
			sql = new CustomerRecurringPaymentSQL();
			id = random.Next(Int32.MinValue, -1);
			rowState = RowState.New;			
		}
		public CustomerRecurringPayment(UOW uow) : this()
		{
			if(uow == null)
		...
		{
			return new CustomerRecurringPaymentSQL();
		}
		...
		public static CustomerRecurringPayment find(UOW uow, int id)
		{
			if (uow.Imap.keyExists(CustomerRecurringPayment.getKey(id)))
				return (CustomerRecurringPayment)uow.Imap.find(CustomerRecurringPayment.getKey(id));
            
			CustomerRecurringPayment cls = new CustomerRecurringPayment();
			cls.uow = uow;
			cls.id = id;
			cls = (CustomerRecurringPayment)DomainObj.addToIMap(uow, getOne(((CustomerRecurringPaymentSQL)cls.Sql).getKey(cls)));
			cls.uow = uow;
            
			return cls;
		}
		public static CustomerRecurringPayment[] getAll(UOW uow)
		{
			CustomerRecurringPayment[] objs = (CustomerRecurringPayment[])DomainObj.addToIMap(uow, (new CustomerRecurringPaymentSQL()).getAll(uow));
			for (int i = 0; i < objs.Length; i++)
				objs[i].uow = uow;
			return objs;
		}
		public static CustomerRecurringPayment[] GetCustROPByAccount(UOW uow, int accNumber)
		{
			CustomerRecurringPayment[] objs = (CustomerRecurringPayment[])DomainObj.addToIMap(uow, (new CustomerRecurringPaymentSQL()).GetCustROPByAccount(uow, accNumber));
			for (int i = 0; i < objs.Length; i++)
				objs[i].uow = uow;
			return objs;
		}
		...
		static CustomerRecurringPayment getOne(CustomerRecurringPayment[] acls)
		{
			if (acls.Length == 1)
				return acls[0];
		...
		static void copyAttrs(CustomerRecurringPayment src, CustomerRecurringPayment tar)
		{
			if (tar == null)
		...
		#region	SQL
		[Serializable]
			class CustomerRecurringPaymentSQL : SqlGateway
		{
			public CustomerRecurringPayment[] getKey(CustomerRecurringPayment rec)
			{
				SqlCommand cmd = getCommand(rec);
				cmd.CommandText = "spCustomerRecurringPayment_Get_Id";
				cmd.Parameters.Add("@Id", SqlDbType.Int, 0).Value = rec.id;
				return convert(execReader(cmd));
			}			
			public CustomerRecurringPayment[] GetCustROPByAccount(UOW uow, int accNumber)
			{
				SqlCommand cmd = makeCommand(uow);
				cmd.CommandText = "spCustomerRecurringPayment_Get_ByAccount";
				cmd.Parameters.Add("@AccNumber", SqlDbType.Int, 0).Value = accNumber;
				return convert(execReader(cmd));
			}		
			public override void insert(DomainObj obj)
			{
				CustomerRecurringPayment rec = (CustomerRecurringPayment)obj;
				SqlCommand cmd = getCommand(rec);
				cmd.CommandText = "spCustomerRecurringPayment_Ins";
				setParam(cmd, rec);
			...
			public override void delete(DomainObj obj)
			{
				CustomerRecurringPayment rec = (CustomerRecurringPayment)obj;
				SqlCommand cmd = getCommand(rec);
				cmd.CommandText = "spCustomerRecurringPayment_Del_Id";
				cmd.Parameters.Add("@Id", SqlDbType.Int, 0).Value = rec.id;
			...
			public override void update(DomainObj obj)
			{
				CustomerRecurringPayment rec = (CustomerRecurringPayment)obj;
				SqlCommand cmd = getCommand(rec);
				cmd.CommandText = "spCustomerRecurringPayment_Upd";
				setParam(cmd, rec);
                
				execScalar(cmd);
				rec.rowState = RowState.Clean;
			}
			public CustomerRecurringPayment[] getAll(UOW uow)
			{
				SqlCommand cmd = makeCommand(uow);
				cmd.CommandText = "spCustomerRecurringPayment_Get_All";
				return convert(execReader(cmd));
			}
			#region Implementation
			void setParam(SqlCommand cmd, CustomerRecurringPayment rec)
			{
				cmd.Parameters.Add("@Id", SqlDbType.Int, 0).Value = rec.id;
			...
			{
				CustomerRecurringPayment rec = new CustomerRecurringPayment();
			...
			CustomerRecurringPayment[] convert(DomainObj[] objs)
			{
				CustomerRecurringPayment[] acls  = new CustomerRecurringPayment[objs.Length];
				objs.CopyTo(acls, 0);
				return  acls;
			}
			#endregion
