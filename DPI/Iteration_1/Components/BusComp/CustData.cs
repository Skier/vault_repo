...
/*			Since ICustBalanceInfo is removed...
	...
	*/
		...
		public static CustData[] getPhone(UOW uow, string phNumber)
		{
			...
			try
			{
				Int64 i = Int64.Parse(phNumber.Trim());
			}
			catch (Exception e)
			{
        ...
        static void copyAttrs(CustData src, CustData tar)
        {
            if (tar == null)
			...
			tar.webPassowrd = src.webPassowrd;
...
//			}
			...
			ICustDataValidation reader2(SqlDataReader rdr)
			{
				...
				if ( rdr["WebPassword"] != DBNull.Value)
					cd.webPassowrd = (string) rdr["WebPassword"];
            ...
            void setParam(SqlCommand cmd, CustData rec)
            {
				...
				if (rec.webPassowrd == null)
					cmd.Parameters.Add("@WebPassword", SqlDbType.VarChar, 25).Value = DBNull.Value;
				else
				{
					if (rec.webPassowrd.Length == 0)
						cmd.Parameters.Add("@WebPassword", SqlDbType.VarChar, 25).Value = DBNull.Value;
					else
						cmd.Parameters.Add("@WebPassword", SqlDbType.VarChar, 25).Value = rec.webPassowrd;
				}
            }
            ...
            protected override DomainObj reader(SqlDataReader rdr)
            {
				...
				if (rdr["WebPassword"] != DBNull.Value)
					rec.webPassowrd = (string) rdr["WebPassword"];
