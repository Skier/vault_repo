...
This email confirms that you have modified your account information using www.dpiteleconnect.com
...
phone: 1-800-350-4009";
        ...
        public static bool IsEligibleForPromiseToPay(IMap imap, int accNumber)
        {
            ...
            try {
                ...
                return CustData.IsEligibleForPromiseToPay(uow, accNumber);
            } finally {
                uow.close();
            }
        }
        ...
        public static bool DoesPromiseToPayExist(IMap imap, int accNumber)
        {
            ...
            try {
                ...
                return CustData.DoesPromiseToPayExist(
        ...
        public static PromiseToPay GetPromiseToPay(IMap imap, int accNumber) 
        {
            ...
            try {
                ...
                IAcctInfo acctInfo = Account.GetAccountInfo(uow, accNumber);
                return CustData.GetPromiseToPay(
                    uow, accNumber, acctInfo.DueDate);
            } finally {
                uow.close();
            }
        ...
        public static DateTime GetPromiseToPayDate(IMap imap, int accNumber)
        {
            ...
            try {
                ...
                return CustData.GetPromiseToPayDate(uow, accNumber);
            } finally {
                uow.close();
            }   
        }
        ...
        public static void ChangeAccountSettings(IMap imap, int accNumber, string email, string webPassword)
        {
            if (imap == null) {
                throw new ArgumentException("imap is required");
            }
            ...
            try {
                ...
                msg.AddEmailTo(custData.Email);
                if (oldEmail != null && oldEmail.Length > 0 && oldEmail != custData.Email) {
                    msg.AddEmailTo(oldEmail);
                }
                msg.EmailFrom = SERVICE_EMAIL_FROM;
                ...
                msg.EmailMessage = string.Format(
                    ...
                    CustInfo.CapitalizeName(custData.CustomerInfo.FirstName));
        ...
        public static void EnableWebAccess(IMap imap, int accNumber, string accLastName, string email, string webPassword)
        {
            if (imap == null) {
                throw new ArgumentException("imap is required");
            }
            ...
            try {
                ...
                if (accountInfo.LastName == null || (
                    ...
                    throw new ArgumentException("Account Last Name is invalid.");
                }
        ...
        public static WebValidationResultStatus RemindWebPassword(IMap imap, int accNumber)
        {
            UOW uow = null;
            ...
            IMap imap, int accountNumber, DateTime payDate, decimal payAmount, string userId)
        {
            ...
            try {
                ...
                CustData.MakeIvrRecord(uow, accountNumber, payDate, payAmount, userId);
            } finally {
                uow.close();
            }
        }
