...
namespace Dpi.Central.Web.Account.Payment
{
    public class PaymentSelectionPage : BasePaymentPage
    {
        ...
        private void vldCstAmt_ServerValidate(object source, System.Web.UI.WebControls.ServerValidateEventArgs args) 
        {
            ...
            {                
                args.IsValid = false;
                vldCstAmt.ErrorMessage = "Please select a Payment Type to continue with the processing of your payment.";
            ...
            } else
            {
                try {
                    ...
                    if (amount < min) {
                        args.IsValid = false;
                        vldCstAmt.ErrorMessage = "The minimum payment amount accepted is " + min.ToString("C");
                    } else if (amount > max) {
                        args.IsValid = false;
                        vldCstAmt.ErrorMessage = "The maximum payment amount accepted is " + max.ToString("C");
                    } else {
                        args.IsValid = true;
                    }
                } catch {
