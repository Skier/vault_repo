...
and to keep you informed of web access activity on your account.";
        ...
        {
            string body = string.Format(NEW_ACCOUNT_EMAIL_BODY, CustInfo.CapitalizeName(firstName));
...
This email confirms that your account settings have been modified using www.dpiteleconnect.com.";
        ...
        {
            string body = string.Format(ACCOUT_CHANGE_NOTIFICATION_EMAIL_BODY, CustInfo.CapitalizeName(firstName));
...
scheduled to be received for no later than {2}.";
        ...
        {
            string body = string.Format(PROMISE_TO_PAY_BODY, CustInfo.CapitalizeName(firstName), payAmount.ToString("C"), payDate.ToString("MMM dd, yyyy", DateTimeFormatInfo.CurrentInfo));
...
the amount of {2} on {3}. The approval number for the payment transaction is {4}.";
        ...
        {
            string body = String.Format(PAYMENT_BODY, CustInfo.CapitalizeName(firstName), "Credit Card", payAmount.ToString("C"), DateTime.Now.ToShortDateString(), paymentId);
        ...
        {
            string body = String.Format(PAYMENT_BODY, CustInfo.CapitalizeName(firstName), "Check", payAmount.ToString("C"), DateTime.Now.ToShortDateString(), paymentId);
            SendMessage(emailAddress, PAYMENT_SUBJECT, body);
        }
