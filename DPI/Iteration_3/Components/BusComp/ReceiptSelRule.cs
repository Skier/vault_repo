...
namespace DPI.Components
{
    ...
    public class ReceiptSelRule : DomainObj
    {
		...
		public static int GetReceiptId(UOW uow, string prodGroup, int prod, int wlProd, 
									   int corp, string store, string workflow, bool isCompleted)
		{
	        return getOne(new ReceiptSelRuleSQL()
					.getReceiptId(uow, prodGroup, prod, wlProd, corp, store, workflow, isCompleted)).ReportId;
		}
        ...
        class ReceiptSelRuleSQL : SqlGateway
        {
			...
			public ReceiptSelRule[] getReceiptId(UOW uow,  string prodGroup, int prod, int wlProd, int corp, 
				string store, string workflow, bool isCompleted)
			{
				SqlCommand cmd = makeCommand(uow);
				cmd.CommandText = "spReceiptSelRuleGetReceiptId";
				setPatternParams(cmd, prod, prodGroup, wlProd, store, corp, workflow, isCompleted);
				return convert(execReader(cmd));
			}

            /*        Implementation        */
			void setPatternParams(SqlCommand cmd, int prod, string prodGroup,  int wlProd, string store, int corp, 
