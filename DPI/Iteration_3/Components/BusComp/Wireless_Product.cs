...
namespace DPI.Components
{
	...
	public class Wireless_Products : DomainObj, IWireless_Products
	{
		...
		{
			return new Wireless_ProductsSQL().GetDpiWLMainProds(uow, isRecharge, isInternational);
...
//			return objs;
		...
		public static Wireless_Products[] GetDpiWLOptionalProds(UOW uow)
		{
			Wireless_Products[] objs = (Wireless_Products[])DomainObj.addToIMap(uow, (new Wireless_ProductsSQL()).GetDpiWLOptionalProds(uow));
			for (int i = 0; i < objs.Length; i++)
				objs[i].uow = uow;
			return objs;
		}
			...
			class Wireless_ProductsSQL : SqlGateway
		{
			...
			public Wireless_Products[] GetDpiWLOptionalProds(UOW uow)
			{
				SqlCommand cmd = makeCommand(uow);
