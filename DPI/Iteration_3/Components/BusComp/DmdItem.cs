...
namespace DPI.Components
{
	...
	#region Constructors
		...
		public DmdItem(UOW uow, IWireless_Products wp, IDemand demand) : this(uow)
		{
			if (wp == null)
				throw new ArgumentNullException("Wireless_Products is required");

			if (demand == null)
				throw new ArgumentNullException("Demand is required");

			this.Setup(wp, demand);			
		}
		...
		void Setup( IWireless_Products wp, IDemand demand)
		{
			this.prod        = wp.Wireless_product_id;
			this.packageId   = wp.Wireless_product_id;
			this.ParDemand   = demand;
			this.priceAmt    =  this.effPrice = Decimal.Round(wp.Price, 2);	
			this.status      = "Pending";
		}
	#endregion	
...
//		}
		...
		void CompWLTax(UOW uow, DmdItem di)
		{
			di.taxes = DmdTax.ConvDomObj(uow, di, new EzTaxProxy().GetTaxProxy().ComputeTax(di.Prod, di.effPrice, true, Zipcode(uow), DateTime.Now)); 
		}
