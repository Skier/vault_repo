...
namespace DPI.Services
{
	...
	{
        public static IProdPrice[] GetTopProd(IMap imap, int ilecId, string zipcode, string role, 
            string storeCode, string criteria) 
        {
            ...
            try {
                ...
                IProdPrice[] prods 
                    = ProdQual.FilterTopProducts(uow, AvailProds.GetAvailProds(uow, ilecId, zipcode, role, storeCode));
            ...
            string storeCode, string criteria) 
        {
            ...
            return GetTopProd(imap, ilec.OrgId, zipcode, role, storeCode, criteria);
        }
		...
		public static IDmdTax[] ComputeTax(IMap imap, int prod, decimal priceAmt, bool isWireless, string zip, DateTime date)
		{
			UOW uow = null;	
			IBillSoftTax btax = null;
			...
			try
			{
				...
				return btax.ComputeTax(uow, prod, priceAmt, isWireless, zip, date);				
			}
			finally
			{
				imap.ClearDomainObjs();
        ...
        public static ProdComposition[] GetProductsComposition(IMap imap) 
        {
            UOW uow = null;	

            try {
                uow = new UOW(imap, "ProdSvc.GetProductsComposition");	

                return ProdComposition.getAll(uow);
            }
            finally {                
                uow.close();
            }	
