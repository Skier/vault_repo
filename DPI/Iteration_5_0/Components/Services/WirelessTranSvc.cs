...
namespace DPI.Services
{
	public class WirelessTranSvc
	{
		...
		public static IWireless_Transactions GetWSTran(IMap imap, string pin, int prodId, string confNum,  string storeCode)
		{
			UOW uow = null;			
			...
			{
				uow = new UOW(imap, "WirelessTranSvc.GetWSTran");			
				return Wireless_Transactions.Locate(uow, pin, prodId, confNum, storeCode);
			}
			finally
			{
				uow.close();
		...
		//				
		//				Wireless_Transactions.SetStatusById(uow, tran_id, trNumber, 
		...
		//			}
		//		}
		public static int SetStatusById(IMap imap, int id, string trNumber, string status)
		{
			UOW uow = null;
			try
			{
				uow = new UOW(imap, "WirelessTranSvc.SetStatusById");
				return Wireless_Transactions.SetStatusById(uow, id, trNumber, status);
			}
			finally
			{
				uow.close();
		...
		public static int PostPendWLTran(IMap imap, int prod, string storeCode, string clerkId, string pin, decimal webProdPrice)
		{
			UOW uow = null;
			try
			{
				uow = new UOW(imap, "WirelessTranSvc.PostPendWLTran");
				return Wireless_Transactions.PostPendWirelessTran(uow, prod, storeCode, clerkId, pin, webProdPrice, new Random().Next(1, 999999).ToString());
			}
			finally
			{
				uow.close();
