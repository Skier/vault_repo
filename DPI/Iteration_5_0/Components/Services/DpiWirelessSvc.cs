...
namespace DPI.Services
{
	public class DpiWirelessSvc : ISvcProvider
	{
        ...
        public static IWireless_Products[] GetFinalProducts(IMap imap, int subProductId) 
        {
            if (imap == null) {
            ...
            try {
                uow = new UOW(imap, "DpiWirelessSvc.GetFinalProducts");
                return Wireless_Products.GetFinalProducts(uow, subProductId);
            } finally {
                uow.close();
            }
        }
        public static IWireless_Products[] GetSubProducts(IMap imap, int finalProductId) 
        {
            if (imap == null) {
                throw new ArgumentNullException("imap");
            }
            ...
            try {
                uow = new UOW(imap, "DpiWirelessSvc.GetSubProducts");
                return Wireless_Products.GetSubProducts(uow, finalProductId);
            } finally {
                uow.close();
            }
        }
		...
		{ 			
			...
			try
			{
                ...
                decimal transactionAmount = GetTransactionAmount(payInfo, taxAmt);
                decimal commissionAmount = GetCommissionAmount(wp, transactionAmount);

				CreateWirelessTran(uow, user, customer, payInfo, taxAmt, commissionAmount, wp, cell);
		...
		public static ICellPhoneReceipt Replenish(IMap imap, IUser user, string provider, IWireless_Custdata customer, IPayInfo payInfo, decimal taxAmt, ICellPhoneInfo cell, IWireless_Products wp)
		{
            ...
            return Replenish(imap, user, provider, customer, payInfo, taxAmt, commissionAmount, cell, wp);
		}

        public static ICellPhoneReceipt Replenish(IMap imap, IUser user, string provider, IWireless_Custdata customer, IPayInfo payInfo, decimal taxAmt, decimal commissionAmount, ICellPhoneInfo cell, IWireless_Products wp) 
        {
            UOW uow = null; 
			IWebSvcQueue logQue = null;
			try
		...
		static void CreateWirelessTran(UOW uow, IUser user, IWireless_Custdata customer, IPayInfo payInfo, decimal taxAmt, decimal commissionAmount, IWireless_Products wp, ICellPhoneInfo cell)
		{
			IWireless_Transactions wt = new Wireless_Transactions(uow);
			wt.TrConfirm				= payInfo.Id;
			wt.PayDateTime				= DateTime.Now;
			wt.Tran_Amount				= GetTransactionAmount(payInfo, taxAmt);
			...
			wt.Commission				= commissionAmount;
		...
		static decimal GetCommissionAmount(IWireless_Products wp, decimal tranAmt)
		{
        ...
        static decimal GetTransactionAmount(IPayInfo paymentInfo, decimal taxAmount)
        {
            return paymentInfo.TotalAmountPaid - taxAmount;
        }

        public static string GetSoc(string controlNumber) 
        {
            if (controlNumber.IndexOf("-") == -1) {
                return string.Empty;
            }

            return controlNumber.Substring(0, controlNumber.IndexOf("-"));
        }
