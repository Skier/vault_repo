...
namespace DPI.Services
{
	public class OrgSvc 
	{
        ...
        public static bool DoesZipExist(IMap imap, string zip)
        {
            UOW uow = null;

            try {
                uow = new UOW(imap, "OrgSvc.DoesZipExist");
        ...
        public static IILECInfo[] GetILECs(IMap imap)
        {
            UOW uow = null;

            try {
                uow = new UOW(imap, "OrgSvc.GetILECs");
                return ILECInfo.FindAll(uow);	
            }
            finally {
                uow.close();
            }
