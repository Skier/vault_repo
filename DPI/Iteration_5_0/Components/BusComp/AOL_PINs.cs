...
namespace DPI.Components
{
    ...
    public class AOL_PINs : DomainObj
    {
		...
		public static AOL_PINs ReservePIN(UOW uow, int wirelessProd)
		{
            ...
            try {
                cls = (AOL_PINs)DomainObj.addToIMap(uow, getOne(new AOL_PINsSQL().ReservePIN(uow, wirelessProd)));
            } catch (SqlException ex) {
                if (ex.Class >= 11 && ex.Class <= 16) {
                    throw new PinUnavailableException(wirelessProd);
                }
		...
		public static void ResetPin(UOW uow, string pin)
		{
			new AOL_PINsSQL().ResetPin(uow, pin);
		}
        public static bool IsPinAvailable(UOW uow, int wirelessProductId)
        {
            if (uow == null) {
                throw new ArgumentNullException("uow");
            }

            AOL_PINsSQL ds = new AOL_PINsSQL();

            bool available = ds.IsPinAvailable(uow, wirelessProductId);
        ...
        class AOL_PINsSQL : SqlGateway
        {
			...
			public void ResetPin(UOW uow, string pin)
			{
				SqlCommand cmd = makeCommand(uow);
				cmd.CommandText = "spAOL_ResetPin";
				cmd.Parameters.Add("@PIN", SqlDbType.VarChar, 255).Value = pin;
				execScalar(cmd);
			}
            public bool IsPinAvailable(UOW uow, int wirelessProductId)
            {
                SqlCommand cmd = makeCommand(uow);
                cmd.CommandText = "spAOL_IsPinAvailable";
                cmd.Parameters.Add("@WirelessProductId", SqlDbType.Int).Value = wirelessProductId;
                object pinCount = execScalar(cmd);
                return pinCount != null && ((int)pinCount) > 0;
            }
