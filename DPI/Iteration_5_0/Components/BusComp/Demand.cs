...
namespace DPI.Components
{
	...
	#region	Static methods
		...
		public static Demand BuildDmd(UOW uow, IWireless_Products[] prods, string storeCode, string dmdType)
		{
			Demand dmd = new Demand(uow, dmdType);
			dmd.Loc = Location.find(uow, StoreLocation.find(uow, storeCode).Zip).LocId;
			dmd.Status = DemandStatus.Pend.ToString();
			dmd.storeCode = storeCode;
	...
	#region	Implementation
		...
		static bool IsTaxingReq(string storeCode)
		{
			if (storeCode == null)
				return false;

			return StoreStatsCol.GetCorporation(storeCode).IsDpiWirelessTax;			
		}
	#endregion
