	...
	public class Wireless_Products : DomainObj, IWireless_Products
	{
		...
		public string Vendor_Name
		{
			...
			{ 
				setState();
				vendor_Name = value; 
			} //no setState since we will not save in DB
		}
		public string Soc
		{
			get { return soc; }
		...
		public static Wireless_Products[] GetDpiWLMainProds(UOW uow, string provider, bool isRecharge, bool isInternational)
		{
			return new Wireless_ProductsSQL().GetDpiWLMainProds(uow, provider, isRecharge, isInternational);
...
//			return objs;
		...
		public static Wireless_Products[] GetDpiWLOptionalProds(UOW uow, string provider)
		{
			Wireless_Products[] objs = (Wireless_Products[])DomainObj.addToIMap(uow, (new Wireless_ProductsSQL()).GetDpiWLOptionalProds(uow, provider));
			for (int i = 0; i < objs.Length; i++)
				objs[i].uow = uow;
			return objs;
		}
		public static Wireless_Products[] GetDpiEnergyProducts(UOW uow, string storeCode, string provider, bool isNew)
		{
			Wireless_Products[] objs = (Wireless_Products[])DomainObj.addToIMap(uow, (new Wireless_ProductsSQL()).GetDpiEnergyProducts(uow, storeCode, provider, isNew));
			for (int i = 0; i < objs.Length; i++)
				objs[i].uow = uow;
			return objs;
		}
		public static Wireless_Products[] GetPackageByProdList(UOW uow, string prodList)
		{
			Wireless_Products[] objs = (Wireless_Products[])DomainObj.addToIMap(uow, (new Wireless_ProductsSQL()).GetPackageByProdList(uow, prodList));
			for (int i = 0; i < objs.Length; i++)
				objs[i].uow = uow;
			return objs;
		}
		public static Wireless_Products[] GetProdsBySoc(UOW uow, string soc, bool isActivation)
		{
			Wireless_Products[] objs = (Wireless_Products[])DomainObj.addToIMap(uow, (new Wireless_ProductsSQL()).GetProdsBySoc(uow, soc, isActivation));
			for (int i = 0; i < objs.Length; i++)
				objs[i].uow = uow;
			return objs;
		}
        ...
        public static Wireless_Products[] GetFinalProducts(UOW uow, int subProductId)
        {
            Wireless_ProductsSQL ds = new Wireless_ProductsSQL();
            Wireless_Products[] finalProducts = ds.GetFinalProducts(uow, subProductId);
            return finalProducts;
        }
        public static Wireless_Products[] GetSubProducts(UOW uow, int finalProductId)
        {
            Wireless_ProductsSQL ds = new Wireless_ProductsSQL();
            Wireless_Products[] products = ds.GetSubProducts(uow, finalProductId);
            return products;
        }
		#endregion        
			...
			class Wireless_ProductsSQL : SqlGateway
		{
			...
			public Wireless_Products[] GetDpiWLMainProds(UOW uow, string provider, bool isRecharge, bool isInternational)
			{
				SqlCommand cmd = makeCommand(uow);
			...
			public Wireless_Products[] GetDpiWLOptionalProds(UOW uow, string provider)
			{
				SqlCommand cmd = makeCommand(uow);
			...
			public Wireless_Products[] GetDpiEnergyProducts(UOW uow, string storeCode, string provider, bool isNew)
			{
				SqlCommand cmd = makeCommand(uow);
				cmd.CommandText = "spWirelessGetDpiEnergyProducts";
				cmd.Parameters.Add("@Storecode", SqlDbType.VarChar, 10).Value = storeCode;
				cmd.Parameters.Add("@Provider", SqlDbType.VarChar, 25).Value = provider;
				cmd.Parameters.Add("@IsNew", SqlDbType.Bit, 0).Value = isNew;
			
				return execReader1(cmd);
			}
			public Wireless_Products[] GetPackageByProdList(UOW uow, string prodList)
			{
				SqlCommand cmd = makeCommand(uow);
				cmd.CommandText = "spWirelessGetPackageByProductList ";				
				cmd.Parameters.Add("@ProductList", SqlDbType.VarChar, 200).Value = prodList;		
			
				return execReader1(cmd);
			}
			public Wireless_Products[] GetProdsBySoc(UOW uow, string soc, bool isActivation)
			{
				SqlCommand cmd = makeCommand(uow);
				cmd.CommandText = "spWirelessGetPlan ";				
				cmd.Parameters.Add("@SOC", SqlDbType.VarChar, 50).Value = soc;
				cmd.Parameters.Add("@IsActivation", SqlDbType.Bit, 0).Value = isActivation;
            ...
            public Wireless_Products[] GetFinalProducts(UOW uow, int subProductId) 
            {
                SqlCommand cmd = makeCommand(uow);
                cmd.CommandText = "spWirelessGetFinalProductsBySubProduct";
                cmd.Parameters.Add("@SubProductId", SqlDbType.Int, 0).Value = subProductId;

                return convert(execReader(cmd));
            }
            public Wireless_Products[] GetSubProducts(UOW uow, int finalProductId) 
            {
                SqlCommand cmd = makeCommand(uow);
                cmd.CommandText = "spWirelssGetSubProductsByFinalProduct";
                cmd.Parameters.Add("@FinalProductId", SqlDbType.Int, 0).Value = finalProductId;
			...
			Wireless_Products reader2(SqlDataReader rdr)
			{
				Wireless_Products rec = new Wireless_Products();
