...
namespace DPI.Components
{
	...
	#region CustInfo Properties
		...
		static void copyAttrs(CustInfo src, CustInfo tar)
		{
			...
			tar.activationDate = src.activationDate;
		}
			...
			class CustInfoSQL : SqlGateway
		{
			...
			void setParam(SqlCommand cmd, CustInfo rec)
			{
				...
				if (rec.activationDate == DateTime.MinValue)
					cmd.Parameters.Add("@ActivationDate", SqlDbType.DateTime).Value = DBNull.Value;
				else
					cmd.Parameters.Add("@ActivationDate", SqlDbType.DateTime).Value = rec.activationDate;
			...
			protected override DomainObj reader(SqlDataReader rdr)
			{
				...
				if (rdr["ActivationDate"] != DBNull.Value)
					rec.activationDate = (DateTime) rdr["ActivationDate"];
