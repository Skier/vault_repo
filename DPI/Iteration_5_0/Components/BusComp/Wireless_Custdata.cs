...
namespace DPI.Components
{
    ...
    #region	Static methods
        ...
        public static IWireless_Custdata[] GetByPhone(UOW uow, string phone) 
        {
            Wireless_Custdata[] objs = (Wireless_Custdata[])DomainObj.addToIMap(uow, (new Wireless_CustdataSQL()).GetByPhone(uow, phone));
            for (int i = 0; i < objs.Length; i++)
                objs[i].uow = uow;
            return (IWireless_Custdata[])objs;
        }
    ...
    #region	SQL
        ...
        class Wireless_CustdataSQL : SqlGateway
        {
			...
			public Wireless_Custdata[] GetByPhoneOrEsn(UOW uow, string phoneOrEsn)
			{
				...
				cmd.Parameters.Add("@PhoneOrEsn", SqlDbType.VarChar, 25).Value = phoneOrEsn;

				return convert(execReader(cmd));
			}
            public Wireless_Custdata[] GetByPhone(UOW uow, string phone)
            {
                SqlCommand cmd = makeCommand(uow);
                cmd.CommandText = "spWireless_Custdata_Get_ByPhone";
                cmd.Parameters.Add("@PhNumber", SqlDbType.VarChar, 25).Value = phone;
