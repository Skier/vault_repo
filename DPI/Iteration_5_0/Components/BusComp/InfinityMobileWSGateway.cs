...
{
	public class InfinityMobileWSGateway  : IWebSvcProvider
	{
	#region Const
		string provider;
	#endregion		
	...
	#region Properties
		...
		static string Acct { get { return System.Configuration.ConfigurationSettings.AppSettings["InfMobileWebSvcAcct"]; }}
		static string PW   { get { return System.Configuration.ConfigurationSettings.AppSettings["InfMobileWebSvcPwd"];  }}
		public	bool IsWirelessXactPosted { get { return false; }}
	#endregion

	#region Constructors
		public InfinityMobileWSGateway(string provider)
		{
	...
	#region Methods
		...
		{
			throw new ApplicationException("InfinityMobileWSGateway.GetProducts() is not implemented");
		}
...
//			}
		...
		{
			return new PinReceipt(GetPIN(uow, wp.Wireless_product_id), 0m, "");
		}
		...
		{
			XmlNode xNode = new PaceWS().ActivatePhone(ci.NewESN, ci.Pin, ci.Zip, "", Acct, PW); 
		...
		ICellPhoneReceipt CheckActivation(IUOW uow, ICellPhoneInfo ci, IWebSvcQueue logQue)
		{
			...
			XmlNode xNode = new PaceWS().CheckActivation(ci.NewESN, Acct, PW); 
		...
		ICellPhoneReceipt ReplenishServicePlan(IUOW uow, ICellPhoneInfo ci, IWebSvcQueue logQue)
		{
			...
			XmlNode xNode = new PaceWS().ReplenishServicePlan(ci.Pin, ci.PhoneNumber, Acct, PW); 
		...
		public SvcPlanDataResp GetSvcPlanData(IUOW uow, ICellPhoneInfo ci, IWebSvcQueue logQue)
		{
			...
			XmlNode xNode = new PaceWS().GetServicePlanData(ci.PhoneNumber, ci.NewESN, startDate, endDate, Acct, PW); 
		...
		{
			throw new ApplicationException("GetAvailableBalance method is not implemented in InfinityMobileWSGateway");
		}
	#endregion
