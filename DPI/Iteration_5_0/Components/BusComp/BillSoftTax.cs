...
namespace DPI.Components
{
	...
	#region Methods
		...
		public IDmdTax[] ComputeTax(IUOW uow, int prod, decimal priceAmt, bool isWireless, string zip, DateTime date) // add logging
		{
			TaxCode tc = null;

			if (isWireless)
				tc  = TaxCode.find((UOW)uow, ((IWireless_Products)Wireless_Products.find((UOW)uow, prod)).TaxCode);
			else
				tc  =  ProdInfoCol.GetTaxCode(prod);
	...
	#region Implementation
		...
		zip_address GetAddress(UOW uow, BillSoftArg args)
		{
			...
			if (addr.state_abv == "PR")
				addr.country_ISO = "PRI";
			else
