...
{
	public class TelispireWSGateway  : IWebSvcProvider
	{
	#region Const
		string provider;
	#endregion		
	...
	#region Properties
		...
		static string Acct { get { return ConfigurationSettings.AppSettings["TelispireWebSvcAcct"]; }}
		static string PW   { get { return ConfigurationSettings.AppSettings["TelispireWebSvcPwd"];  }}
		public	bool IsWirelessXactPosted { get { return false; }}
	#endregion

	#region Constructors
		public TelispireWSGateway(string provider)
		{
	...
	#region Methods
		...
		{
			throw new ApplicationException("TelispireWSGateway.GetProducts() is not implemented");
		}
...
//			}
		...
		public IReceipt Send(IUOW uow, IUser user, string action, IDomObj[] objects)
		{
			...
			throw new ArgumentException("TelispireWSGateway: Uknown action '" + action + "'");
		}
		...
		public bool ResetPin(IWebSvcQueue logQue, string pin)
		{
			try
			{
				XmlNode xNode = new TelispireWS().ResetPin(pin, Acct, PW);			
		...
		{
			XmlNode xNode = new TelispireWS().GetServicePlanList("", esn, Acct, PW);
		...
		public ICellPhoneReceipt ActivatePhone(ICellPhoneInfo ci, IWebSvcQueue logQue)
		{
			try
			{
				...
				XmlNode xNode = new TelispireWS().ActivatePhone(ci.NewESN, ci.Pin, ci.Zip, "", Acct, PW);			
			...
			{
				ICellPhoneReceipt rcpt = new CellPhoneReceipt(false, ex.Message + " TelispireWSGateway.ActivatePhone failed.");
				rcpt.Pin = ci.Pin;
		...
		public ICellPhoneReceipt ReplenishServicePlan(IUOW uow, ICellPhoneInfo ci, IWebSvcQueue logQue)
		{
			try
			{
				XmlNode xNode = new TelispireWS().ReplenishServicePlan(ci.Pin, ci.PhoneNumber, Acct, PW); 
			...
			{
				ICellPhoneReceipt rcpt = new CellPhoneReceipt(false, ex.Message + " TelispireWSGateway.ReplenishServicePlan failed.");
				rcpt.Pin = ci.Pin;
...
//		}
		void ProcessPlan(ICellPhoneReceipt rct, ICellPhoneInfo ci, IWebSvcQueue logQue)
		{
			...
			{
				Thread.Sleep(10000);
				xNode = new TelispireWS().GetWirelessDeviceData("", ci.NewESN, Acct, PW);
		...
		public ICellPhoneReceipt CheckActivation(ICellPhoneInfo ci, IWebSvcQueue logQue)
		{
			...
			{
				Thread.Sleep(10000);
				xNode = new TelispireWS().CheckActivation(ci.NewESN, Acct, PW);
		...
		{
			return new CheckActivationResp(new TelispireWS().CheckActivation(esn, Acct, PW));
		}
		...
		public ISvcPlanDataResp GetDpiSvcPlanData(string phone, string esn)
		{
			...
			XmlNode xNode = new TelispireWS().GetServicePlanData(phone, esn, startDate, endDate, Acct, PW); 
		...
		public ISvcPlanDataResp GetAvailableBalance(string phone, string esn)
		{
			try
			{
				XmlNode xNode = new TelispireWS().GetAvailableBalance(phone, esn, Acct, PW); 
		...
		public IWirelessDeviceData GetWLDeviceDataResp(string phone, string esn)
		{
			try
			{
				XmlNode xNode = new TelispireWS().GetWirelessDeviceData(phone, esn, Acct, PW); 

				IWirelessDeviceData resp = new WirelessDeviceDataResp(xNode);
				resp.Provider = Const.TELISPIRE;
				return resp;
			}
			catch (Exception ex)
			{
		...
		public ISvcPlanDataResp GetSvcPlanData(IUOW uow, ICellPhoneInfo ci, IWebSvcQueue logQue)
		{
			...
			XmlNode xNode = new TelispireWS().GetServicePlanData(ci.PhoneNumber, ci.NewESN, startDate, endDate, Acct, PW); 
		...
		public bool DeactivatePhone(IWebSvcQueue logQue, string phone)
		{
			try
			{
				IWirelessDeviceData resp = GetWLDeviceDataResp(phone, null);
				...
				XmlNode xNode = new TelispireWS().DeactivatePhone(phone, Acct, PW);				
				logQue.InitRespXml = xNode.InnerXml;
