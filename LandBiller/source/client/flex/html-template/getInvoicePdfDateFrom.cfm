...
<cfscript>
    invoiceFileName = getDirectoryFromPath(currentPath) & "ExpenseFiles\Invoice_" & URL.invoiceId & ".pdf";
    mergedPdfFileName = getDirectoryFromPath(currentPath) & "ExpenseFiles\Invoice_" & URL.invoiceId & "_merged.pdf";
    finalPdfFileName = getDirectoryFromPath(currentPath) & "ExpenseFiles\Invoice_" & URL.invoiceId & "_final.pdf";
</cfscript>

<cfquery name="selectAttachments" datasource="LL_Expence">
    SELECT    ll_invoiceitem.invoiceitemid, ll_invoiceitem.itemdate, <cfqueryparam value="#storageDirectory#" cfsqltype="CF_SQL_VARCHAR"> || ll_file.storagekey as filename
...
<cfreport template="InvoiceDateFrom.cfr" format="pdf" query="selectInvoice" 
