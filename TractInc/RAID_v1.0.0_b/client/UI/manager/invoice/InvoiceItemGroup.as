package UI.manager.invoice
{
	...
	public class InvoiceItemGroup extends EventDispatcher
	{
		...
		public function setSelected(selected:Boolean):void 
		{
			...
			for each (var item:InvoiceItem in items) {
				item.IsSelected = selected;
			}
		}
		
		public function addItem(item:InvoiceItem):void 
		{
			if (item == null) {
				return;
			}
		...
		public function removeItem(item:InvoiceItem):void 
		{
			if (item == null) {
				return;
			}
		...
		private function existsInDeleted(item:InvoiceItem):Boolean {
			for each (var i:InvoiceItem in deletedItems) {
		...
		private function onItemChanged(event:* = null):void 
		{
			...
			for each (var item:InvoiceItem in items) {
				...
				if (item.IsSelected) {
					...
					if (item.InvoiceItemTypeId == 1) {  // incorrect  !!!!!!
						totalDays += item.Qty;
