package UI.crew
{
	...
	public class SummaryItem extends EventDispatcher
	{
			...
			for each (var billItem:BillItem in bill.RelatedBillItem) {
				...
				if (bill.Status == BillStatus.BILL_STATUS_CORRECTED 
					&& !(billItem.Status == BillItemStatus.BILL_ITEM_STATUS_APPROVED 
						...
						|| billItem.Status == BillItemStatus.BILL_ITEM_STATUS_VERIFIED
						) ) {
					return false;
				}
			}
		...
		{
			for each ( var billItem:BillItem in bill.RelatedBillItem) {
				if ( billItem.StatusTemp.Status != BillItemStatus.BILL_ITEM_STATUS_APPROVED
					&& billItem.StatusTemp.Status != BillItemStatus.BILL_ITEM_STATUS_REJECTED 
					&& billItem.StatusTemp.Status != BillItemStatus.BILL_ITEM_STATUS_CONFIRMED ) {
   					canSubmit = false;
   					return;
    			}
    		}
