package App.Login
{
    ...
    public class LoginController
    {
        ...
        private function onLoginOk(event:ResultEvent):void {
            var user:User = ActiveRecords.User.findByPrimaryKey(User(event.result).UserId);
            if (!user.IsLoaded) {
            	user.addEventListener("loaded", OnUserLoaded);
            } else {
            	mainApp.OnUserLoggedIn(user);
            }
        }

        private function OnUserLoaded(evt:DynamicLoadEvent): void {
            User(evt.data).removeEventListener("loaded", OnUserLoaded);
            mainApp.OnUserLoggedIn(User(evt.data));
        }
