...
namespace TractInc.TrueTract.Data
{
    internal class GroupDataMapper
    {
        ...
        private const string SQL_SELECT = @"
            SELECT [TT_Group].GroupId, [TT_Group].GroupName
              FROM [TT_Group]
        ";
        ...
        private const string SQL_SELECT_BY_USER_ID = SQL_SELECT + @"
            INNER JOIN TT_GroupUser on TT_GroupUser.GroupId = [TT_Group].GroupId
            WHERE TT_GroupUser.UserId = @UserId
        ";

        private const string SQL_CREATE = @"
            INSERT INTO [TT_Group] ( GroupName ) 
                 VALUES ( @GroupName )
