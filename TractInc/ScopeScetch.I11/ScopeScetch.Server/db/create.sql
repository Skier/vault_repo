...
if exists (select * from dbo.sysobjects where id = object_id(N'[SyncLog]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
drop table [SyncLog]
...
create table DocumentType (
    ...
    Name            varchar (50),
    SellerRoleName  varchar (50),
    BuyerRoleName   varchar (50) 
) 
...
create table SyncLog(
  SyncLogId     int         not null identity constraint PK_SyncLog primary key,
  UserId        int         not null,
  DeviceId      varchar(50) not null,
  IsDirty       bit         not null constraint DF_SyncLog_IsDirty default (1),
  SyncTimeStamp binary(8)   not null 
)
...
alter table SyncLog add
  constraint FK_SyncLog_User foreign key(UserId) references [User](UserId)
...
alter table Permission add
  constraint FK_Permission_Module1 foreign key(ModuleId) references Module(ModuleId) on update cascade on delete cascade
