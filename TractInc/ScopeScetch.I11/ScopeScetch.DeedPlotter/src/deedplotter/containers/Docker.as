...
public class Docker extends VBox
{
    ...
    override public function get viewMetricsAndPadding():EdgeMetrics
    {
  			...
  			finalPlacement:Boolean = false):Boolean
	{
		...
		pt = globalToLocal(pt);
		var toolbar:VBox;
		if (pt.y < topBar.height + 15 && pt.y > topBar.y - 15)
			toolbar = topBar;
		...
		if (toolbar)
		{
			...
			if (finalPlacement)
			{
                ...
                if (item.docked) {
    				...
    				if (row == p) {
    				    var itemIndex:Number = p.getChildIndex(item);
    				    if (itemIndex == colIndex) return false;
    				    else if (itemIndex < colIndex) colIndex--;
    				}
