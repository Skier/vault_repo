...
namespace TractInc.TrueTract.Data
{
    internal class DocDataMapper
    {
        ...
        public DocumentInfo GetDocumentById(SqlTransaction tran, int docId) {
            ...
            if (null != documentInfo)
            {
                List<DocAttachmentInfo> attachments = AttachmentDM.GetAttachmentsByDocId(tran, documentInfo.DocID);
                documentInfo.Attachments = attachments.ToArray();
            }
        ...
        public DocumentInfo Create(SqlTransaction tran, DocumentInfo documentInfo, int userId) {
            ...
            {
                foreach (DocAttachmentInfo attachment in documentInfo.Attachments)
                {
                    attachment.DocId = documentInfo.DocID;
                    AttachmentDM.Create(tran, attachment);
                }
            }
