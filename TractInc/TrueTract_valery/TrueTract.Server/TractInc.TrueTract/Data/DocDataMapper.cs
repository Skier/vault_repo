...
//        private const string DOCUMENT_IS_NOT_UNIQUE = "Document with such unique fields (State, County, DocType, DocNu, Volume, Page) is already exists";
        ...
        private const string SQL_SELECT = @"
            SELECT DISTINCT
              ...
              FROM [Document]
				inner join State s on s.StateId = [Document].State
				inner join County c on c.CountyId = [Document].County
                left join
        ...
        private const string SQL_SELECT_ACTUAL_BY_DOC_ID = SQL_SELECT + @" 
             WHERE [Document].DocBranchUid IN ( SELECT DocBranchUid 
                                                  FROM [Document] 
                                                 WHERE [Document].DocId = @DocId ) 
        ...
        private const string SQL_SELECT_BY_GROUP_AND_USER = SQL_SELECT + @"
                INNER JOIN [GroupItem] on [GroupItem].DocBranchUid = [Document].DocBranchUid
                INNER JOIN [GroupUser] on [GroupUser].GroupId = [GroupItem].GroupId
             WHERE [GroupUser].GroupId = @GroupId
               AND [GroupUser].UserId = @UserId
        ";

        private const string SQL_SELECT_BY_PROJECT = SQL_SELECT + @"
	            inner join ProjectTabDocument td on [Document].DocId = td.DocumentId 
	            inner join ProjectTab pt on td.ProjectTabId = pt.ProjectTabId
        ...
        private const string SQL_GET_TYPE_NAME = @"
            SELECT [Name] as DocTypeName FROM [DocumentType]
        ...
        private List<DocumentInfo> Select(SqlTransaction tran, string sql, List<SqlParameter> paramList) {
            ...
            using (SqlDataReader dataReader = SQLHelper.ExecuteReader(tran, CommandType.Text, sql, paramList.ToArray())) {
                ...
                while (dataReader.Read()) {
                    ...
                    documentInfo.StateName = dataReader.IsDBNull(dataReader.GetOrdinal("StateName"))
                        ? null : dataReader.GetString(dataReader.GetOrdinal("StateName"));

                    documentInfo.CountyName = dataReader.IsDBNull(dataReader.GetOrdinal("CountyName"))
                        ? null : dataReader.GetString(dataReader.GetOrdinal("CountyName"));
