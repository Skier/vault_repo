...
import src.deedplotter.components.tractViewClasses.TractTextObjectView;
import src.deedplotter.components.tractViewClasses.call.*;
...
[Event(name="callClick", type="src.deedplotter.components.tractViewClasses.events.TractCallEvent")]
[Event(name="callDoubleClick", type="src.deedplotter.components.tractViewClasses.events.TractCallEvent")]
[Event(name="pointMouseDown", type="src.deedplotter.components.tractViewClasses.events.TractPointEvent")]
 ...
 */        }
    ...
    **/
    ...
    public function createTempLine():void 
    {
        ...
        _tempLine.Model.CallOrder = tract.Calls.length;
	...
	public function AddCall(call:TractCall):CallView 
	{
	    ...
	    call.CallOrder = tract.Calls.length;
        ...
        if (tempLine)
        {
            ...
            tempLine.Model.CallOrder = tract.Calls.length;
        }
	...
	public function DeleteCall(call:TractCall):void 
	{
	    ...
	    if (tract.Calls.length > 1) 
	    {
	        if (call.CallOrder == tract.Calls.length - 1) 
	        {
    ...
    public function GetLastCallView():CallView 
    {
        ...
        var lastCall:TractCall = tract.GetCallByOrder(tract.Calls.length - 1);
        if (lastCall) 
        {
	...
	private function rearrangeCallView(needTractAreaRefreshing:Boolean=true):void 
	{
        ...
        for (var i:int = 0; i < tract.Calls.length; i++) 
        {
            var call:TractCall = tract.Calls[i];
            call.CallOrder = i;
        ...
        if (tract.Calls.length > 0 && callStartPoint.startPosition.Equals(startPoint.startPosition)) 
        {
    ...
    **/
    ...
    {
        tract.Calls.sort = new Sort();
        tract.Calls.sort.fields = [ new SortField("CallOrder", false, false, true) ];
        tract.Calls.refresh();
    }
